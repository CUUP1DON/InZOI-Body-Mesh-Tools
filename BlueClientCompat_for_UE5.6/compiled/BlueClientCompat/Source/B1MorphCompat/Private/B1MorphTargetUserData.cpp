...
bool UBlueClientMorphTools::SetPostProcessAnimBlueprintByClassPath(USkeletalMesh* TargetMesh, const FString& AnimBlueprintGeneratedClassPath, FString& OutMessage)
{
    ...
    // Direct FSoftObjectPtr assignment bypasses any load attempt and writes
    // the path string directly — it will resolve at runtime when the pak is loaded.
    ...
    // FSoftClassProperty stores an FSoftObjectPtr as its value. Cast directly and
    // assign the path — no load attempted, weak ptr stays null until runtime.
