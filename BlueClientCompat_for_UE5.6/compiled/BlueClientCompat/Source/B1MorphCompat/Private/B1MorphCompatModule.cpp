...
static FString NormalizeMeshPathForLoad(const FString& InPath)
{
    ...
    // If the path still has junk before /Game/ (e.g. a leading " that Trim didn't catch,
    // or a BOM), slice from the first /Game/ so StartsWith checks and LoadObject work.
    ...
    // Trim trailing junk (second arg, merged quotes, etc.)
    ...
    // Accept either an object path (/Game/.../SKM.SKM) or a package path (/Game/.../SKM)
...
IMPLEMENT_MODULE(FB1MorphCompatModule, B1MorphCompat)

void FB1MorphCompatModule::StartupModule()
{
    ImportConsoleCommand = IConsoleManager::Get().RegisterConsoleCommand(
        ...
        FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
        {
            ...
            // Join remaining args with spaces to reconstruct paths that contain spaces
            ...
            if (bSuccess)
            {
                ...
                UE_LOG(LogTemp, Display, TEXT("[BlueClientMorphTools] SAVE YOUR MESH NOW — open it and press Ctrl+S, or use File > Save All."));
            }
            else
            {
...
void FB1MorphCompatModule::ShutdownModule()
{
