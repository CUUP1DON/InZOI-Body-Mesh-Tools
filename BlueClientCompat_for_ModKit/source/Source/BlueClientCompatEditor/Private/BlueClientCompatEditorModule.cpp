#include "BlueClientCompatEditorModule.h"
...
/** Slate loads plugin PNGs reliably when rooted under Resources/ (not via raw FSlateImageBrush paths). */
...
IMPLEMENT_MODULE(FBlueClientCompatEditorModule, BlueClientCompatEditor)

void FBlueClientCompatEditorModule::StartupModule()
{
    ...
    // Unregister first: avoids duplicate Window menu entries if Startup runs again without Shutdown (e.g. hot reload).
    ...
    FTabSpawnerEntry& Spawner = FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
        ...
        FOnSpawnTab::CreateRaw(this, &FBlueClientCompatEditorModule::SpawnTransferTab))
...
void FBlueClientCompatEditorModule::ShutdownModule()
{
    FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(BlueClientCompatTransferTabName);
...
TSharedRef<SDockTab> FBlueClientCompatEditorModule::SpawnTransferTab(const FSpawnTabArgs& SpawnTabArgs)
{
