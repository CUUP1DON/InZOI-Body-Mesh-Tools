...
void SBlueClientCompatTransferWidget::Construct(const FArguments& InArgs)
{
    ...
    const TSharedRef<SVerticalBox> TransferBody = SNew(SVerticalBox)
        ...
        [
            SNew(SBorder)
                .BorderBackgroundColor(FLinearColor(1.f, 0.85f, 0.f, 0.15f))
                .Padding(FMargin(8.f, 6.f))
            [
                SNew(STextBlock)
                    .Text(FText::FromString(TEXT("Use only one JSON path at a time. If both are filled, the Custom Body JSON takes priority and the Original Body JSON will be ignored.")))
                    .Font(HintFont())
                    .ColorAndOpacity(FSlateColor::UseSubduedForeground())
                    .AutoWrapText(true)
            ]
        ...
        [
            ...
            [
                SAssignNew(JsonPathEdit, SEditableTextBox)
                    .HintText(FText::FromString(TEXT("Original body JSON file path")))
                    .Font(LabelFont())
            ]
            + SHorizontalBox::Slot()
                  .AutoWidth()
            ...
            [
                SNew(SButton)
                    .Text(FText::FromString(TEXT("Browse Original Body JSON")))
        ...
        [
            ...
            + SHorizontalBox::Slot()
                  ...
                  .VAlign(VAlign_Center)
            [
                SAssignNew(BlenderJsonPathEdit, SEditableTextBox)
                    .HintText(FText::FromString(TEXT("Custom body JSON file path (from Body Mesh Tools Blender addon)")))
                    .Font(LabelFont())
            ]
            + SHorizontalBox::Slot()
                  .AutoWidth()
            ...
            [
                SNew(SButton)
                    .Text(FText::FromString(TEXT("Browse Custom Body JSON")))
                    .OnClicked(this, &SBlueClientCompatTransferWidget::OnBrowseBlenderJsonClicked)
            ]
        ]
        + SVerticalBox::Slot()
              .AutoHeight()
...
FReply SBlueClientCompatTransferWidget::OnTransferClicked()
{
    ...
    // Priority 1: Blender shape key JSON — most accurate, no source mesh needed
    const FString BlenderJsonPath = BlenderJsonPathEdit.IsValid() ? BlenderJsonPathEdit->GetText().ToString() : FString();
    if (!BlenderJsonPath.IsEmpty())
    {
        if (!FPaths::FileExists(BlenderJsonPath))
        {
    ...
    {
        // Fallback: verbatim copy from original inZOI JSON (same-topology meshes only)
        const FString JsonFilePath = JsonPathEdit.IsValid() ? JsonPathEdit->GetText().ToString() : FString();
        if (JsonFilePath.IsEmpty())
        {
            StatusText = FText::FromString(TEXT("No JSON path set. Browse a Blender JSON or an original inZOI body JSON."));
            return FReply::Handled();
        }
...
FReply SBlueClientCompatTransferWidget::OnBrowseBlenderJsonClicked()
{
    bLastTransferOk = false;
    IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
    if (!DesktopPlatform)
    ...
    const bool bOpened = DesktopPlatform->OpenFileDialog(
        const_cast<void*>(ParentWindowHandle),
        TEXT("Select Blender shape key JSON (from blender_export_morph.py)"),
        DefaultPath,
        TEXT(""),
        TEXT("JSON files (*.json)|*.json"),
        EFileDialogFlags::None,
    ...
    {
        const FString& OutFilePath = OutFilenames[0];
        if (BlenderJsonPathEdit.IsValid())
        {
            BlenderJsonPathEdit->SetText(FText::FromString(OutFilePath));
        }
        StatusText = FText::FromString(FString::Printf(TEXT("Selected Blender JSON: %s"), *OutFilePath));
    }
    else
    {
        StatusText = FText::FromString(TEXT("Blender JSON selection cancelled."));
    }
